...
function renderList(data) {
    ...
    data.forEach(transaction => {
        ...
        const amountClass = transaction.type === 'income' ? 'text-primary' : 'text-danger';
        trElm.innerHTML = `
        <td>${transaction.date}</td>
        <td>${transaction.item}</td>
        <td class="${amountClass}">${transaction.amount}</td>
        <td class="${amountClass}">${transaction.type === 'income' ? '収入' : '支出'}</td>
        `;
        listElement.appendChild(trElm);
    });
}
...
function calculateSummary(data) {
    ...
    data.forEach(data => {
        if (data.type === 'income') {
            incomeTotalNum += data.amount;
        } else if (data.type === 'expense') {
            expenseTotalNum += data.amount;
        }
    });

    let balanceTotalNum = incomeTotalNum - expenseTotalNum;

    incomeTotalArea.textContent = incomeTotalNum;
    expenseTotalArea.textContent = expenseTotalNum;
    balanceTotalArea.textContent = balanceTotalNum;
...
function filterTransactions(type) {
    ...
    if (type === 'all') {
        ...
        let filterList = transactions.filter(transaction => transaction.type === type);
...
function setupEventListeners() {
    ...
    filterButtonArray.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            } else if (e.target.id === 'btn-income') {
